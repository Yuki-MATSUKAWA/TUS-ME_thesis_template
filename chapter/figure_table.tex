...
図~\ref{fig:tiger_comparison} では (\subref{subfig:tiger_eps}) EPS ファイル，(\subref{subfig:tiger_pdf}) PDF ファイル，(\subref{subfig:tiger_png}) PNG ファイルを並べて比較しています．
パネル (\subref{subfig:tiger_eps}, \subref{subfig:tiger_pdf}) はどちらもベクター画像なのでいくら拡大しても明瞭なままですね．
一方のパネル (\subref{subfig:tiger_png}) を拡大するとラスター画像なので小さな正方形で構成されていることが確認できます．
...
\begin{figure}[tp]
    ...
    \begin{subfigure}{0.32\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{figure/tiger.eps}
        \caption{EPS ファイル（\texttt{tiger.eps}）．}
        \label{subfig:tiger_eps}
    \end{subfigure}
    \hfill % ここで空白を入れると図が適切に配置される
    \begin{subfigure}{0.32\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{figure/tiger.pdf}
        \caption{PDF ファイル（\texttt{tiger.pdf}）．}
        \label{subfig:tiger_pdf}
    \end{subfigure}
    \hfill % ここで空白を入れると図が適切に配置される
    \begin{subfigure}{0.32\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{figure/tiger.png}
        \caption{PNG ファイル（\texttt{tiger.png}）．}
        \label{subfig:tiger_png}
    \end{subfigure}
    \caption{3 枚の図．}
    \label{fig:tiger_comparison}
\end{figure}
